...
void split(size_t size, node_t **previous, node_t **free_block,
  ...
  *allocated = (header_t *)(*free_block);
  size_t actual_size = size - sizeof(node_t) + sizeof(header_t);
  size_t prev_size = (*free_block)->size;
  *free_block = (node_t *)(((char *)*free_block) + actual_size);
  (*free_block)->size = prev_size - size - sizeof(header_t);
  if(*previous != NULL)
...
void my_free(void *allocated) {
  ...
  node_t *node = (node_t *)allo;
  node->size = allo->size - sizeof(header_t) + sizeof(node_t);
  node->next = head;
  head = node;
  coalesce(head);
}
